...
function matches(row, where = {}) {
  return Object.entries(where).every(([k,v]) => k === 'OR' ? v.some(part => matches(row,part)) : k === 'AND' ? v.every(part => matches(row,part)) : v && Array.isArray(v.in) ? v.in.includes(row[k]) : equal(row[k], v));
}
function database() {
  const fake = { state: Object.fromEntries(names.map(n => [n, []])), trace: [], hook: null };
      ...
      for (const name of names) {
        ...
        for (const operation of ['findUnique','findMany','create','update','updateMany']) tx[name][operation] = async args => {
          ...
          if (operation.startsWith('find')) {
            ...
            if (args.orderBy) { const key = Object.keys(args.orderBy)[0]; result = result.slice().sort((a,b)=>a[key]-b[key]); }
          ...
          if (operation === 'create') {
            ...
            const unique = { curriculumBook: ['board','grade','subject','title'], curriculumChapter: ['bookId','chapterNumber'], curriculumArtifactVersion: ['artifactChecksum'], curriculumArtifactReview: ['versionId','revision'], curriculumArtifactImport: ['versionId','chapterId'] }[name];
